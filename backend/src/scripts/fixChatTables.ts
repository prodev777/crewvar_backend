...
async function checkAndRecreateChatTables() {
  try {
    ...
    const [tables] = await pool.execute("SHOW TABLES LIKE 'chat_messages'");
    console.log('Chat messages table exists:', (tables as any[]).length > 0);
    ...
    const [roomsTable] = await pool.execute("SHOW TABLES LIKE 'chat_rooms'");
    console.log('Chat rooms table exists:', (roomsTable as any[]).length > 0);
    
    if ((roomsTable as any[]).length > 0) {
      const [roomsColumns] = await pool.execute("DESCRIBE chat_rooms");
    ...
    await pool.execute("DROP TABLE IF EXISTS chat_rooms");
    await pool.execute("DROP TABLE IF EXISTS user_online_status");
    ...
    const [newColumns] = await pool.execute("DESCRIBE chat_messages");

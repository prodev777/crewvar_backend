...
router.get('/connections', authenticateToken, async (req, res) => {
  try {
    ...
    if (!targetShipId) {
      ...
      targetShipId = (userRows[0] as any).current_ship_id;
      if (!targetShipId) {
    ...
    const [connectionsRows] = await pool.execute(`
      SELECT 
...
router.get('/crew-in-port', authenticateToken, async (req, res) => {
  try {
    const userId = (req as any).user?.userId;
    ...
    const userShipId = (userRows[0] as any).current_ship_id;
    if (!userShipId) {
    ...
    const [linkedShipsRows] = await pool.execute(`
      SELECT DISTINCT 
        linked_ship_id,
        port_name
      FROM port_linkings 
    ...
    const [connectedShipsRows] = await pool.execute(`
      SELECT DISTINCT 
        CASE 
    ...
    const linkedShipIds = (linkedShipsRows as any[]).map(row => row.linked_ship_id);
    const connectedShipIds = (connectedShipsRows as any[]).map(row => row.connected_ship_id).filter(id => id !== null);
    ...
    console.log('📊 Response data:', {
      crewCount: (crewRows as any[]).length,
      linkedShips: linkedShipIdsOnly.length,
      portName: (linkedShipsRows as any[])[0]?.port_name || null
    });
...
// Link ships in port
router.post('/link-ships', authenticateToken, async (req, res) => {
  try {
    const userId = (req as any).user?.userId;
    ...
    `, [uuidv4(), userId, userShipId, portName || null, date]);
...
router.get('/available-ships', authenticateToken, async (req, res) => {
  try {
    const userId = (req as any).user?.userId;
    const { date = new Date().toISOString().split('T')[0] } = req.query;
    ...
    const [shipsRows] = await pool.execute(`
      SELECT DISTINCT
        s.id,
        s.name,
        cl.name as cruise_line_name,
...
// Unlink ships
router.delete('/unlink-ships/:shipId', authenticateToken, async (req, res) => {
  try {
    const userId = (req as any).user?.userId;
...
async function checkAndCreateShipConnection(ship1Id: string, ship2Id: string, date: string) {
  try {
    ...
    const [ship1ToShip2Count] = await pool.execute(`
    ...
    const [ship2ToShip1Count] = await pool.execute(`
      SELECT COUNT(DISTINCT pl.linking_user_id) as count
      FROM port_linkings pl
      JOIN users u ON pl.linking_user_id = u.id COLLATE utf8mb4_unicode_ci
      WHERE u.current_ship_id = ? 
    ...
    const count1to2 = (ship1ToShip2Count as any[])[0]?.count || 0;
    const count2to1 = (ship2ToShip1Count as any[])[0]?.count || 0;
...
async function createShipConnectionNotifications(ship1Id: string, ship2Id: string, date: string) {
  try {
    console.log('🔔 Creating ship connection notifications');
    
    // Get all crew members from both ships
    const [ship1Crew] = await pool.execute(`
      SELECT id, display_name FROM users WHERE current_ship_id = ? AND is_active = true
    `, [ship1Id]);
    
    const [ship2Crew] = await pool.execute(`
      SELECT id, display_name FROM users WHERE current_ship_id = ? AND is_active = true
    `, [ship2Id]);
    
    // Get ship names for notification
    const [ship1Info] = await pool.execute(`SELECT name FROM ships WHERE id = ?`, [ship1Id]);
    const [ship2Info] = await pool.execute(`SELECT name FROM ships WHERE id = ?`, [ship2Id]);
    
    const ship1Name = (ship1Info as any[])[0]?.name || 'Ship 1';
    const ship2Name = (ship2Info as any[])[0]?.name || 'Ship 2';
    
    // Create notifications for ship1 crew about ship2 connection
    for (const crew of (ship1Crew as any[])) {
      try {
        await createNotification(
          crew.id,
          'ship_connected',
          'Ships Docked Together',
          `Your ship ${ship1Name} and ${ship2Name} are docked together! You can now see all crew members from both ships.`,
          { ship1Id, ship2Id, connectionDate: date }
        );
    ...
    for (const crew of (ship2Crew as any[])) {
      try {
        await createNotification(
          crew.id,
          'ship_connected',
          'Ships Docked Together',
          `Your ship ${ship2Name} and ${ship1Name} are docked together! You can now see all crew members from both ships.`,
          { ship1Id, ship2Id, connectionDate: date }
        );
      } catch (notificationError) {
        console.error('Failed to create notification for crew:', crew.id, notificationError);

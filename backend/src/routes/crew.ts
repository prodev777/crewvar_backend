...
router.get('/onboard', authenticateToken, async (req, res) => {
    try {
        ...
        const userShip = (userRows as any[])[0];
        ...
        console.log('Number of crew rows found:', (crewRows as any[]).length);
        
        // Debug: Log each crew member's data
        (crewRows as any[]).forEach((row, index) => {
...
router.get('/profile/:userId', authenticateToken, async (req, res) => {
    try {
        ...
        const currentUserShipId = (currentUserRows as any[])[0]?.current_ship_id;
        ...
        if ((connectionRows as any[]).length > 0) {
            ...
            const [requestRows] = await pool.execute(
                'SELECT status FROM connection_requests WHERE requester_id = ? AND receiver_id = ?',
        ...
        if (user.additional_photo_1) additionalPhotos.push(user.additional_photo_1);
        if (user.additional_photo_2) additionalPhotos.push(user.additional_photo_2);
        if (user.additional_photo_3) additionalPhotos.push(user.additional_photo_3);
